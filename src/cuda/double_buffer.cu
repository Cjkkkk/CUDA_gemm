...
template <
    ...
    > 
__global__ void DoubleBufferMatMul( 
    ...
    const int A_SM_OFFSET = 0;
    const int B_SM_OFFSET = 0;
    // can not unroll since K can not be determined at this point
    for (int tile_idx = 0 ; tile_idx < K ; tile_idx += BLOCK_SIZE_K) {
        // load A from global memory to shared memory
        #pragma unroll
        for ( int i = 0 ; i < BLOCK_SIZE_M ; i += A_TILE_ROW_STRIDE) {
            FETCH_FLOAT4(As[A_TILE_ROW_START + i + A_SM_OFFSET][A_TILE_COL]) = FETCH_FLOAT4(A[OFFSET(
        ...
        for ( int i = 0 ; i < BLOCK_SIZE_K; i += B_TILE_ROW_STRIDE) {
            FETCH_FLOAT4(Bs[B_TILE_ROW_START + i + B_SM_OFFSET][B_TILE_COL]) = FETCH_FLOAT4(B[OFFSET(
        ...
        for (int k = 0; k < BLOCK_SIZE_K; ++ k) {
            ...
            for (int thread_y = 0; thread_y < THREAD_SIZE_Y; ++thread_y) {
                frag_a[thread_y] = As[ty * THREAD_SIZE_Y + thread_y + A_SM_OFFSET][k];
            }

            // load B from shared memory to register
            #pragma unroll
            for (int thread_x = 0; thread_x < THREAD_SIZE_X; thread_x += 4) {
                FETCH_FLOAT4(frag_b[thread_x]) = FETCH_FLOAT4(Bs[k + B_SM_OFFSET][THREAD_SIZE_X * tx + thread_x]);
            }
        ...
        // change offset so new block of A and B will not override the previous data
        // therefore __syncthreads() is not needed;
        A_SM_OFFSET ^= BLOCK_SIZE_M;
        B_SM_OFFSET ^= BLOCK_SIZE_K;
    }
